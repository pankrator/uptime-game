...
1. Fold every placed workload's `demands` onto its server (`usedByServer`) — a server can
   host several workloads at once, so this is a sum, not a single lookup. Also
   accumulates each server's extra cooling draw from placed workloads.
...
3. Roll online machines up into their rack's `RackLoad` (power, heat, server count) and
   into the facility's `traitsTotal`/`traitsFree`.
...
- `RackLoad.heatKw`'s workload-cooling addition mirrors `resource.ts`'s `drawFor` exactly
  — keep both in sync if the cooling-bonus formula changes. See [resource](./resource.md).
