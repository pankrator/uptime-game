...
// 40px — touch-target floor. Desktop mouse users lose nothing from the larger button; one
// shared geometry beats a second, mobile-only layout.
...
// 44px — touch-target floor. Every offset below that reads HUD_BAR_HEIGHT (drawTopBar's midY,
// getGameViewportRect's top, the offers/workload panels' y) shifts consistently since none of
// them hardcode the old value.
...
// 34px fits comfortably within the 44px HUD_BAR_HEIGHT with margin to spare.
...
// the floor on touch), the touch equivalent of pressing Space. Left of the mute button, same
// row/size family.
...
// server fits this" warning line above the buttons (see hud.ts's drawOfferCard). Servable
// cards just leave that line's space blank, same as a pending workload row always reserving
// its shortfall line whether or not there's a shortfall to show. Worst case the card's own
// text runs to an 80px offset (penalty + recurring + "no server fits this"); the buttons need
...
// Offers panel — a modal opened by the `O` key (job-panels.ts). Each offer keeps rendering at
// its own stable SLOT (0..MAX_OFFERS-1, assigned once at spawn — entities.ts's spawnOffer /
// Offer.slot) rather than a position in a sorted-by-id array: positional indexing would make
// every other card shift up (and the pointer land on the wrong card) the instant an earlier
// offer expired.
...
// a generous upper bound, since slots aren't contiguous.
...
// requirement: UNPLACED then ACTIVE, sorted by id, never truncated (it scrolls instead).
...
// — the jobs panel's whole point is showing every stat at once.
...
// Rack panel — opened by clicking a rack: per-rack, not per-server, so moving a workload
// between servers is a single drag; viewable without travel, drags gated on arrival.
// Centered overlay rather than pinned to a HUD edge: it needs room for up to
// RACK_SLOT_CAPACITY server rows, each with TRAIT_KEYS.length trait bars, plus a tray strip —
// more content than any existing HUD panel.
...
// Sized for four stacked bar rows (three traits + wear, reusing getServerTraitBarRect at
// traitIndex === TRAIT_KEYS.length — see render.ts) plus a row of repair/decommission buttons
// beneath it. Layout, top to bottom: 16px to the server-name label, 14px to the power/cooling
// draw line, then a gap, then four stacked bar rows (each a label line + a 6px bar, 20px
// apart), a 6px gap, an 18px action-button row, 8px bottom margin. See getServerRowLabelY /
// getServerRowDrawY / getServerTraitBarRect / getServerActionButtonRect below for the exact
// offsets this height is sized against — keep them in sync if this changes.
...
// 24px — a second line (progress bar + time-remaining, mirroring the HUD workload panel's
// "active" rows in hud.ts) needs room beyond the original single text line.
export const RACK_CHIP_HEIGHT = 24;
export const RACK_CHIP_GAP = 3;
// Wide enough for "Render Farm 2/2 🔥0.8" (label + cycle counter + cooling-bonus chip text) at
// 9px sans-serif without truncating. Shared by getServerTraitBarRect (as its chip-column
// reservation) and getPlacedChipRect — keep them equal.
export const RACK_CHIP_WIDTH = 112;
export const RACK_TRAY_HEADER_HEIGHT = 20;
// Fits 4 lines (label+cycle, demands, deadline, penalty).
export const RACK_TRAY_CARD_WIDTH = 130;
export const RACK_TRAY_CARD_HEIGHT = 50;
export const RACK_TRAY_CARD_GAP = 8;
// 32px — touch-target floor.
...
// apart — RACK_SERVER_ROW_HEIGHT's comment is sized against exactly these numbers, so change
// them together.
...
// Repair/decommission buttons — a row beneath the trait+wear bars. Repair sits left,
// decommission right, both bottom-aligned in the row (RACK_SERVER_ROW_HEIGHT's comment is
// sized against this).
...
// "Abandon this contract" button — a small square overlaid in the tray card's top-right
...
// One computed layout object instead of many independent functions that each re-derive the
// panel rect from scratch (drawing a single server row would otherwise evaluate
// getRackPanelRect many times with identical arguments and results). This is the single source
// every rect below is derived from; the exported get*Rect functions further down are thin
// wrappers over it, kept so every existing call site (and layout.test.ts) keeps working
// unchanged.
...
// top, one row per catalog entry in the selected category, a buy button per row.
export const SHOP_PANEL_WIDTH = 420;
export const SHOP_PANEL_PADDING = 16;
// Header/row/button sizes sized to the touch-target floor — row text is positioned by
// fraction of row.height (render.ts), so it re-centers safely.
...
// Toast stack — centered under the HUD bar so it reads as "something just happened". Doesn't
// need to dodge the offers/workload panels since those are toggled modals, not permanently
// docked chrome.
...
// The world-drawing/camera-framing area, minus the top HUD bar. The offers/workload panels
// are toggled modals, not permanently-docked chrome, so there's no fixed column width to
// reserve — the play area fills the canvas below the bar.
...
export function pointerInHud(point: { x: number; y: number }, canvas: HTMLCanvasElement): boolean {
  ...
  // dpr-scaled backing-buffer resolution set by rendering/index.ts and would misalign every
  // rect below against a CSS-pixel pointer position.
  ...
  // The tutorial banner is deliberately NOT blocked here: it's an overlay, not an interactive
  // panel, and its own two hit targets (action button, skip link) are already checked ahead of
  // everything else in input.ts's click chain. Blocking the world underneath it would make
  // anything the banner happens to sit over — including the shop door — unreachable by click
  // for as long as the banner is up.
  return false;
}

// Tutorial banner — a persistent, always-on-top overlay (drawn last, by hud.ts) explaining the
// current guided-tutorial step. Docked to the canvas's BOTTOM-RIGHT corner so it never plants a
// near-opaque box in the middle of the play area — exactly where a top-down game gets the most
// click-to-move traffic. Not the bottom-left corner either — that's the build panel's column
// (see getBuildPanelHeight/minX below). Not docked under the HUD bar either — the camera clamps
// the world's top edge to a fixed screen position, so anything docked there would cover the
// same world location (e.g. the shop door) for the whole tutorial. Its own action/skip buttons
// are hit-tested in input.ts, ahead of everything else in the priority chain, same treatment as
// the mute button.
