...
// Wear gained this tick — only ever called for a machine that is online (wear accrues only
...
// themselves (never roll per-tick, or the effective rate depends on UPDATE_HZ and the current
// time-control scale).
...
// A repair buys time, never resets to new — recovering less than full wear is what keeps
